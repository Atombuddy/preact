...
class App extends preact.Component {
  ...
  onSendButton(chatbox) {
   ...
   // chatbox = new Chatbox();
    ...
    Socketizer(user, room) {
        const socket = io.connect("http://127.0.0.1:6060");
      
        // when it connects it emits username and roomid
        socket.on("connect", function () {
          ...
          document.getElementById("message_input_form").onsubmit = function (e) {
            ...
            if (message.length) {
              ...
              // add the message to chatbot text
              const newnode = document.createElement("div");
              // newnode.innerHTML = `<b>${user}: </b> ${message}`;
              newnode.innerHTML = `${message}`;
              newnode.classList.add("messages__item", "messages__item--operator");
              document.getElementById("message").appendChild(newnode);
            }
      
            // message_input value empty
            message_input.value = "";
            message_input.focus();
          };
        });
        ...
        // alert of someone joined the room
        socket.on("join_room_announcement", function (data) {
          ...
          newnode.classList.add("chatbox__announce");
          document.getElementById("message").appendChild(newnode);
        });
      
        // alert of someone left the room
        ...
        // handler for receive message event of socketIO
        socket.on("receive_message", function (data) {
          ...
          newnode.classList.add("messages__item", "messages__item--visitor");
          document.getElementById("message").appendChild(newnode);
        });
    }

    //Socketizer(username, room)
	...
	componentDidMount() {
        ...
        this.state = false;
        this.messages = [];
        this.socket = io.connect("http://127.0.0.1:6060");
		    this.display()
        this.Socketizer("Nazeer",1)
	...
	render() {
		return (
<div>
      <h1 style="margin: 10px;">Welcome to Beta Testing</h1>
      <img src="./images/female.png" width="170px" height="70px" />
        <div class="container">
          <div style={this.chatbox} class="chatbox">
            <div class="chatbox__support">
              <div class="chatbox__header">
                <div class="chatbox__image--header">
                  <img src="./images/female.png" alt="image" />
                </div>
                <div class="chatbox__content--header">
                  <h4 class="chatbox__heading--header">You smart bargaining assistant</h4>
                  <p class="chatbox__description--header" onSubmit="">
                    <h12>Powered by NeyX</h12>
                  </p>
                </div>
              </div>
              <div id="message" class="chatbox__messages"></div>
              <div class="chatbox__footer">
                <form id="message_input_form">
                  <input type="text" id="message_input" placeholder="Write a message..." autocomplete="off" />
                  <button class="chatbox__send--footer send__button" type="submit">Send</button>
                </form>
              </div>
            </div>
            <div class="chatbox__button">
              <button>
                <img src="./images/logo.svg" width="70px" height="70px" />
              </button>
            </div>
          </div>
        </div>
